...
public class ApiUrl {
    ...
    private void autoInitRealToolUrl() {
        ...
        // 如果配置的是独立toolUrl，通常是指向python版本link，此时我们需要先试探一下，python版本的服务是否启动；如果没有启动，则使用workflow的服务；否则直接使用python
        // link服务
     ...
     * 获取工具服务地址
     * <p>
     * - 在这里做了一个降级逻辑，当配置的toolUrl 是 workflow
     * 时，则直接使用workflow，否则尝试使用python版本link，如果python版本link启动了，则使用python版本link，否则使用workflow的地址
     *
     * @return
     */
