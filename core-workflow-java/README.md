__IntelligentFlow 是一个企业级的 AI 工作流编排平台，能让用户通过可视化拖拽的方式，把大模型、语音合成、各种插件工具串成一条自动化流水线，不用写代码就能构建自己的 AI 应用。类似 n8n、扣子、dify 等平台。
...
StateGraph和CompiledGraph的关系：StateGraph是图的定义阶段需要在这个阶段声明有哪些节点、节点之间怎么连接、什么条件走什么分支，它是可变的，我们可以不断往里面添加东西。而 CompiledGraph 是调用 compile() 方法之后得到的"可执行图"，它是不可变的，专门用来执行的。
编译这一步的步骤是：验证图的结构是否合法，比如有没有孤立节点、有没有死循环风险；把节点和边的定义转换成内部的执行结构；如果配置了 checkpointer，还会初始化状态持久化的相关逻辑。拿到 CompiledGraph 之后，就可以通过 invoke() 或者 stream() 方法来执行。而且 CompiledGraph 是线程安全的，可以被多个请求并发调用，不需要每次都重新构建图。

AgentState是LangGraph4J 中用来承载整个工作流执行状态的核心对象，贯穿整个图的执行过程，每个节点都可以读、修改，然后把修改后的状态传递给下一个节点。
    跨节点数据传递：节点A的输出可以通过 AgentState 传递给节点B。
    全局上下文共享：存储整个工作流执行过程中的共享数据。
    状态持久化基础：配合 CheckpointSaver 实现断点续传。
LangGraph4j推荐使用深拷贝的方式来修改状态，这样配合CheckPointer做状态持久化和回放的时候更可靠。

原本已经有一套 NodeExecutor 的接口体系，后来引入 LangGraph4J 做工作流编排的时候，发现它期望的节点是 AsyncNodeAction<S> 这种函数式接口，接收一个 State 返回一个 State。于是设计了适配器，把 NodeExecutor 包装成 LangGraph4J 能识别的形式，在构建图的时候，通过工厂获取对应的执行器，然后用适配器包一层。

LangGraphWorkflowEngine的执行流程：构建阶段、执行阶段、结果处理阶段。构建阶段接收前端传来的 JSON 配置，解析成 WorkflowConfig 对象。然后 GraphBuilder 分析节点和边的结构，识别出入口节点和出口节点。 接着遍历所有节点，通过 NodeExecutorFactory 获取对应的执行器，再用 NodeExecutorAdapter 把它们适配成 LangGraph4J 能识别的节点动作。最后添加边的连接关系，把入口连到 START，出口连到 END，调用 compile() 得到 CompiledGraph。
执行阶段先初始化 StateManager，把用户的初始输入装进状态里。然后调用 CompiledGraph 的 invoke 方法，LangGraph4J 会按照图的拓扑顺序依次执行各个节点，每个节点执行完会更新状态，状态自动流转到下游节点。如果有条件分支，会根据当前状态动态决定走哪条路径。
结果处理阶段从最终状态里提取 nodeResults，也就是所有节点的执行输出。Output 节点会通过 nodeOutputs 拿到这些数据，按用户配置汇总成最终输出。 同时把执行过程中的关键信息记录到 ExecutionRecord 表里，包括每个节点的输入输出、执行耗时、错误信息等，方便在 DebugDrawer 里展示和排查问题。

NodeAction 是同步执行的，调用的时候当前线程会阻塞，直到节点执行完毕才返回。AsyncNodeAction 是异步执行的，返回的是 CompletableFuture<State>，调用之后立即返回，实际执行在别的线程里进行。

基于 BaseCheckpointSaver 实现检查点机制的作用和实现思路：它解决的核心问题是状态持久化。
    第一是故障恢复，比如一个工作流跑到一半，某个 LLM 节点因为网络问题失败了，如果没有检查点，就得从头开始跑，之前执行过的节点全部浪费。有了检查点，可以从最近一次成功的状态恢复继续执行。
    第二是长对话场景，用户和 Agent 聊了很多轮，对话历史需要持久化，不能服务重启就丢了。
    第三是调试和回放，你想看看工作流在某个节点执行时的状态是什么样的，检查点保存了历史快照可以随时回溯。
实现思路：
    1. 继承LangGraph4j的 BaseCheckpointSaver 抽象类
    2. 在每个节点执行完成后，将当前 AgentState 序列化并持久化（MySQL/Redis）
    3. 恢复时，从存储中读取最近的 Checkpoint，反序列化为 AgentState
    4. 使用恢复的状态重新构建 CompiledGraph 并从断点继续执行(CompiledGraph app = graph.compile(
                                                                            CompileConfig.builder()
                                                                            .checkpointSaver(mySQLCheckpointSaver)
                                                                            .build()
                                                                            );)

**条件分支**是 LangGraph4J 区别于简单 Chain 的核心能力，实现起来主要用 addConditionalEdges 方法。它的思路是：定义一个路由函数，这个函数根据当前状态返回一个字符串，表示下一步要走哪个节点，然后通过映射表把字符串映射到实际的节点名。

基于当前架构实现一个 ReAct Agent，如何设计节点和边的结构：ReAct 是 Reasoning + Acting 的缩写，它的核心思想是让 Agent 在"思考→行动→观察"之间循环，直到得出最终答案。这个模式天然需要循环和条件分支，正是 LangGraph4J 的强项。
节点设计上，我会定义两个核心节点。Agent 节点负责调用大模型进行推理，它会把当前的消息历史（包括之前的思考过程和工具调用结果）发给 LLM，让模型决定下一步是调用工具还是直接给出答案。Tool 节点负责执行工具调用，解析 Agent 输出里的工具指令，调用对应的工具，把结果包装成 Observation 加回消息历史。
边的结构是关键。从 START 进入 Agent 节点，Agent 执行完后通过条件边判断：如果输出里有工具调用指令就走 Tool 节点，如果没有就说明推理完成走 END。Tool 节点执行完后要回到 Agent 节点，形成循环。
有一个细节要注意，就是循环次数限制。万一 LLM 一直觉得需要调工具，就会无限循环，所以要在条件判断里加上**迭代次数检查**，超过阈值强制结束。