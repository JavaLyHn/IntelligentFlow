...
import com.lyhn.coreworkflowjava.link.cache.RedisService;
import com.lyhn.coreworkflowjava.link.constant.LinkConstants;
import com.lyhn.coreworkflowjava.link.constant.LinkErrorCode;
...
import com.lyhn.coreworkflowjava.link.tools.entity.ToolEntity;
import com.lyhn.coreworkflowjava.link.tools.service.ToolCrudService;
...
public class ToolExecutionService {
    ...
    public HttpToolRunResponse httpRun(HttpToolRunRequest runParams) {
        ...
        try {
            ...
            // 如果成功获取到 Schema，就调用 handleRequestExecution 方法进行实际的执行
    ...
    // 调试
    //  ToolDebugRequest 其中包含了要调用的 HTTP 服务的完整信息（ server , method , path , query , header , body ）
    ...
    // 这里会将硬编码的 Python 服务地址 ( core-aitools:18668 ) 替换为当前的 Java 服务地址 ( localhost:7880 )
    ...
    private HttpToolRunResponse handleRequestExecution(
        ...
        try {
            ...
            // 执行http请求
            // 对于 HTTP 类型的工具，它会解析 OpenAPI Schema
