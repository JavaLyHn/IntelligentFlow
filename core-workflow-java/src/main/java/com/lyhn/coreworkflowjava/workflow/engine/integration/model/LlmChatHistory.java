package com.lyhn.coreworkflowjava.workflow.engine.integration.model;
...
public class LlmChatHistory {
    ...
    private static final LoadingCache<String, ConcurrentLinkedQueue<ChatItem>> chatHistoryCache =
            CacheBuilder.newBuilder()
                    ...
                    /**
                        - 当调用 chatHistoryCache.get(key) 时
                        - 如果 key 已存在，直接返回缓存的值
                        - 如果 key 不存在，自动调用 LlmChatHistory.createChatHistoryQueue(key)
                        - 该方法返回一个新的 ConcurrentLinkedQueue<ChatItem> 对象
                        - 将这个新值放入缓存并返回
                     */
