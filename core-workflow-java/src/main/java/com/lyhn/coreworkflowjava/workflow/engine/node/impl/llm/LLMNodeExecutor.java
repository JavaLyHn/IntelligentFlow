...
import com.lyhn.coreworkflowjava.workflow.engine.constants.MsgTypeEnum;
...
import com.lyhn.coreworkflowjava.workflow.engine.domain.chain.OutputItem;
import com.lyhn.coreworkflowjava.workflow.engine.integration.model.LlmChatHistory;
import com.lyhn.coreworkflowjava.workflow.engine.integration.model.ModelServiceClient;
    ...
    protected NodeRunResult executeNode(NodeState nodeState, Map<String, Object> inputs) throws Exception {
        ...
        LlmResVo llmOutput = modelServiceClient.chatCompletion(req, chatResponse -> {
            if (!CollectionUtils.isEmpty(chatResponse.getResults())) {
                // 每收到一小块流式返回的文本 ( text )，就立即调用工作流的 onNodeProcess 事件，把文本实时发出去
