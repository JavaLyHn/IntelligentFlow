...
// 负责具体的工具执行逻辑，它基于解析得到的Schema信息来组装HTTP请求参数并执行工具调用
// 负责把 actionInput 和 businessInput 按照 OpenAPI 的规则把这次调用需要的参数准备好，然后把请求发出去
@Slf4j
public class Tool {
    ...
    public Map<String, Object>[] assembleParameters(Map<String, Object> actionInput,
        ...
        // 从 methodSchema 里拿 parameters 列表，逐个 parameter 去看 in 字段
        // in 是 header 就更新到 header Map，in 是 query 就更新到 query Map，in 是 path 就更新到 path Map
    ...
    public Map<String, Object> assembleBody(Map<String, Object> bodySchema,
        ...
        // 拿到 bodySchema 的 properties 后，会遍历每个字段
        @SuppressWarnings("unchecked")
        ...
        if (bodyProperties != null) {
            for (Map.Entry<String, Object> entry : bodyProperties.entrySet()) {
                ...
                // 如果字段是 object，就递归处理嵌套字段。先从 actionInput 里取出这个字段对应的子对象，取不到就给个空 Map，然后递归调用 assembleBody 继续拼它内部的 properties
